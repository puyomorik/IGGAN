...
def run_eval(config):
    ...
    z_, y_ = utils.prepare_z_y(G_batch_size, G.dim_z, config['n_classes'],
                                ...
                                z_var=config['z_var'])
    get_inception_metrics = inception_utils.prepare_inception_metrics(config['dataset'], config['parallel'], config)

    G.load_state_dict(torch.load(util.open_file_or_url(config['network'])))
