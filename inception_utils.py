...
def prepare_inception_metrics(dataset, parallel, config, no_fid=False):
    ...
    net = load_inception_net(parallel=parallel)
    try:
        moments = np.load('./data/inception/' + dataset + '_inception_moments.npz')
        data_mu = moments['mu']
        ...
        with torch.no_grad():
            for images, _ in loader:
                ...
                pool.append(features.cpu().numpy())
        pool = np.concatenate(pool)
        data_mu = np.mean(pool, axis=0)
        data_sigma = np.cov(pool, rowvar=False)
        if not no_fid:
            np.savez('./data/inception/' + dataset + '_inception_moments.npz', mu=data_mu, sigma=data_sigma)
